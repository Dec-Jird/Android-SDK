...
public class VivoU3DActivity extends UnityPlayerActivity {
	...
	public void onInitEventMainThread(InitEvent event) {
		...
		} else if (str == "getPaymentAction") {
			// 获取活动.
			getPaymentAction(event.getAppid());
		} else if (str == "pay") {
			doSDKPay(event);
...
//		EventBus.getDefault().post(event);
	 ...
	 * @param appid
	 */
	public void getPaymentActionInfo(String appId) {
		InitEvent event = new InitEvent();
	...
	public void getPaymentAction(String appId) {
		log("getPaymentAction: " + appId);
	 ...
	 *            扩展数据
	 */
	public void pay(String appId, String orderNumber, String accessKey, long price,
	...
	public void doSDKPay(InitEvent event) {
		...
		// localBundle.putString("serverName", event.getServiceAreaName()); //服务区名称
